...
CustomImageKeyPair=$(aws ec2 describe-key-pairs --filters Name=key-name,Values=custom-new-key-pair --query 'KeyPairs[0].KeyPairId' --output text)

echo "Get private key from SSM and saving to a file"
aws ssm get-parameter --name /ec2/keypair/$CustomImageKeyPair   --with-decryption --query Parameter.Value --output text > custom-key-pair.pem
...
PublicDnsName=$(aws cloudformation describe-stacks --stack-name my-ec2-instance-from-custom-AMI-stack --query 'Stacks[0].Outputs[?OutputKey == `CustomImageInstancePublicDnsName`].OutputValue' --output text)
